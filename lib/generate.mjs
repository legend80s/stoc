...
function generateCode(itemWithoutCode) {
  ...
  const template = ({
    ...
    return `export async function list(${genParams()}) {
...
async function compileSwaggerJSON(jsonSchema) {
  ...
  for (const [path, value] of Object.entries(jsonSchema.paths)) {
    ...
    for (const [method, value] of methods) {
      ...
      const { parameters, requestBody, responses } = value;
      ...
      function toSchema(parameters) {
        const title = value.summary.replace(/\s/g, '') + 'Params';
      ...
      if (responseSchema.properties) {
        ...
        respBodyData.title =
          ...
          value.summary.replace(/\s/g, '') + 'RespData';
      }
 ...
 */
/**
 * @typedef {Omit<IOpenAPISchema['components']['schemas']['CreateAPIKeyRequest'], 'properties'> & { properties: Record<string, { type: string; title: string; description: string; }>}} ISchema
 */

/**
 * @typedef IGeneratedItemOnlyTypes
 * @property {string} path file path to swagger json
 * @property {string} method
 * @property {string} requestParametersType
