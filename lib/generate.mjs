...
/** @import { IOpenAPISchema, IFilter } from '../lib/typing' */
 ...
 * @param {IOpts} [opts]
 * @returns {Promise<{ list: IGeneratedItem[]; total: int; codeBefore?: string }>} - A promise that resolves to an array of compiled TypeScript definitions.
 */
export async function generateTSFromSchema(jsonSchema, opts = {}) {
  ...
  if (filterConditions) {
    ...
    return { total, list: filtered, codeBefore: requestCode }
  }

  return { list, total, codeBefore: requestCode }
}
...
export async function generateTS(jsonSchema, debug = false) {
  ...
  if (debug) {
    console.log(
      '[debug] after dereference jsonSchema:',
...
async function compileSwaggerJSON(jsonSchema) {
  ...
  for (const [path, value] of Object.entries(jsonSchema.paths)) {
    ...
    for (const [method, value] of methods) {
      ...
      // }
      const respTitle = `${removeWhitespace(summary)}RespData`
      ...
      if (genericResp) {
        // data: { type: 'object', properties: [Object], title: 'listFooRespData' },
        // We should attach the title to data instead of responseSchema thus only one `xxRespData` type will be generated
        // the whole response type will be wrapped in generic type: `ListFooRespData = Data<ListFooRespData>`
      ...
      if (path.includes(testPath)) {
        // responseSchema.title = 'searchProductsRespData'
      }
