...
describe('interfaceToType', () => {
  it('should convert an one-line interface to a type', () => {
    const input =
      'interface IFoo { code: number; message: string | null; data?: IPagedQueryBarsResponseData | null; }'
    const actual = interfaceToType(input)
    const expected =
      'type IFoo = { code: number; message: string | null; data?: IPagedQueryBarsResponseData | null; }'
...
interface IFoo extends IBar { data?: IPagedQueryBarsResponseData | null; }
`
    const actual = interfaceToType(input)
    const expected = `
type IBar = { code: number; message: string | null; }
type IFoo = IBar & { data?: IPagedQueryBarsResponseData | null; }
`
...
it('#addReturnTypeToFunction', () => {
  ...
  assert.equal(
    addReturnTypeToFunction(
      'async function searchProducts(data: ISearchProductsRequestData) {',
      'Promise<ISearchProductsResponseData>'
    ),
    'async function searchProducts(data: ISearchProductsRequestData): Promise<ISearchProductsResponseData> {'
  )
})
